...
Ejercicio 8. Hacemos un bucle while qeu se repite hasta que se pueda dar un resultado válido (termina con break). Construimos el programa mediante try/except. Hacemos un try donde intentamos convertir los inputs a float (y pedir los inputs al usuario) y hacer la divisiómn. Si no puede, el programa da los errores que aparecen en except. El primero si el valor no es númerico  y el segundo si el denominador es 0. Si nohay error, directametne despues de hacer la división muestra el resultado y se termina el blucle. 
...
Ejercicio 36. Creamos la clase UsuarioBlanco donde definimos los atributos nombre, saldo y cuenta_corriente. Definimos la función retirar_dinero, donde lanzamos un error si se intenta retirar más dinero del saldo disponible. Definimos también la función de agregar dinero, donde sumamos la cantidad al saldo. Definoms también la de tranferir_dinero, donde lanzamos un error si el saldo que se quiere transferir es mayor al disponible. Si se puede realizar la tranferencia, sumamos y restamos las cantidades correspondientes de ambos usuarios. 
EN el caso de uso, como el saldo de Alicia es 20 (después de transferirlo a Bob) y se pide retirar 50, nos da un error con un mensaje

Ejercicio 37. Creamos las diferentes funciones: contar_palabras(texto) qeu separa la cadena de texto y devuelve un diccionario cn la palabra y el número de veces que se repite; en reemplazar_palabras usamos la función replace() qeu cambia una palabra por otra; eliminar_palabra() elimina la palabra si aparece más de una vez en el texto y luego vuelve a unirlas en una sola cadena de texto. Estas funciones las unimos todas en la función procesar_texto, qeu escoge una u otra según el argumento que introduzcamos. Esto lo conseguimos con condicionales, y también agregamos los argumentos no válidos mediante el ValueError. 

Ejercicio 38. Primero definimos una función que devuelva una frase u otra según el valor (la hora) que se le introduce. Hacemos un programa de try/except para controlar los errores al pedir los valores por pantalla, de forma que si no es válida la entrada lanza un ValueError, y si es válida llama a la función anteriormente definida.

Ejercicio 39. Se construye de forma al ejercicio anterior. Con la función calificación definimos los resultados que tiene que devolver según el input( lanota) y después pedimos mediante entradas al usuario que ingrese una nota, y si no es válida lanzamos un error. 

Ejercicio 40. Primero importamos el módulo math (para el valro del número pi) . Definmos la función con dos parámetros, y según la figura que sea calcula el área de una forma u otra. Si no tiene los datos suficientes para calcularlo devuelve unan frase explicando el error, y si hay una figura que no sea circulo, triangulo o rectangulo también. 

Ejercicio 41. Realizamos un programa que pide valores al usuario y va verificando las condiciones que se cumplen y controlandolo con mensajes que aparecen por pantalla con estructuras condicionales. Los parámetros que chequea es que el precio sea mayor de 0, que el valor del cupón (si hay cupón) no sea mayor que el precio inicial. Si es un cupón válido hacemos que lo reste del precio inicial, y luego muestra el precio final. Si no es válido no se aplica pero igualmente muestra el precio final. Con el except gestionamos si hay algún ValueError y mostramos un mensaje de que la entrada es inválida
